...
from utils import conv2DBatchNormRelu
...
class TrafficEncoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        batch = obs_traj.size(1) # npeds
        # total = batch * (MAX_PEDS if padded else 1)
        obs_traj_embedding = self.spatial_embedding(obs_traj.contiguous().view(-1,1))
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        if pooling_type:
            ...
            input_dim = 3* encoder_h_dim + bottleneck_dim # 64 + 1024 = 1088 ->64*3+1024=1216
        else:
